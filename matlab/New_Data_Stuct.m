...
for n=3:length(Datensatz_dir)
    ...
    if 'KB23'|bearing_num=='KB24'|bearing_num=='KB27'
        ir_damage=1;
        or_damage=1;
    end
    ...
    if bearing_num=='K002'|bearing_num=='K003'|bearing_num=='K004'|bearing_num=='K005'|bearing_num=='K006'
        damage_type='healthy'; 
        or_damage=0;
        ir_damage=0;
        damage_extent=0;
    ...
    if bearing_num=='KA01'|bearing_num=='KA03'|bearing_num=='KA05'|bearing_num=='KA06'|bearing_num=='KA07'|bearing_num=='KA08'|bearing_num=='KA09'|bearing_num=='KI01'|bearing_num=='KI03'|bearing_num=='KI05'|bearing_num=='KI07'|bearing_num=='KI08'
        ...
        for m=4:(length(bearing_dir)-1)
        new_data_struct(a).bearing_num=bearing_num;
        
        if strncmpi(bearing_dir(m).name,'N09',20)
            new_data_struct(a).rpm=900;
        elseif strncmpi(bearing_dir(m).name,'N15',20)
        ...
        elseif strfind(bearing_dir(m).name,'M01')== 5
         ...
         new_data_struct(a).(bearing_num).damage_type=damage_type;
         new_data_struct(a).(bearing_num).or_damage=or_damage;
         new_data_struct(a).(bearing_num).ir_damage=ir_damage;
         new_data_struct(a).(bearing_num).damage_extent=damage_extent;
...
cd ..
    filepath= pwd;
    filename='new_data_struct';
    fname=[filepath filename];
    save(fname,'new_data_struct','-v7.3','-nocompression');

...
from ml.utils.data import prepare_dataset, load_bearing_data, _prepare_batch
...
def main(raw_args=None):
    ...
    non_blocking = True
    baseline_yaml_path = os.path.join(BEARING_DATA_PATH, "deep_neural_network.yaml")
    ...
    train_set = load_bearing_data(args.class_type, args.signal_type, args.filter, args.bearing_name_train,
                                  args.num_measurement_train, args.segement_size, args.norm_type_input)

    train_data, train_loader, eval_loader, _, num_samples_train = \
        prepare_dataset(train_set[0], train_set[1], args.train_dev_test_split, args.batch_size)

    # Start testing
    if args.enable_testing:
        test_set = load_bearing_data(args.class_type, args.signal_type, args.filter, args.bearing_name_test,
    ...
    # Create DNN
    net_size = [train_data.tensors[0].shape[1]] + args.net_size  # input dimension
    model = create_ffn_net(net_size, args.activations, args.layer_types,
                           args.skip_connections, weight_init=args.weight_init, batch_norm=args.batch_norm,
                           eps=float(args.eps), momentum=args.momentum, dropout=args.dropout)
    model.to(device)

    optimizer = getattr(torch.optim, args.optim)(model.layers.parameters(), lr=float(args.lr))
    ...
    # log model graph
    tb_logger.writer.add_graph(model, torch.zeros((1, args.segement_size)).to(device))
    ...
    hparam_dict["num_parameters"] = model.layers.parameters()
